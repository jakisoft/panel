...
export default () => {
  ...
  const databases = useDeepMemoize(
    ServerContext.useStoreState((state) => state.databases.data)
  );
  const setDatabases = ServerContext.useStoreActions(
    (state) => state.databases.setDatabases
  );
          ...
          <>
            ...
            <Can action={"database.create"}>
              {databaseLimit > 0 && databases.length > 0 && (
                <div css={tw`mt-6 flex text-center justify-center`}>
