  ...
  return (
    <GreyRowBox $hoverable={false} className={"flex-wrap md:flex-nowrap mt-2"}>
      <div className={"flex items-center w-full md:w-auto"}>
        <div className={"pl-4 pr-6 text-neutral-400 hidden md:block"}>
          <FontAwesomeIcon icon={faNetworkWired} />
        </div>
        <div className={"mr-4 md:w-40"}>
          {allocation.alias ? (
        ...
        <div className={"w-14 md:mr-4"}>
          <Code dark className={"w-14 truncate"}>
            {allocation.port}
          </Code>
          <Label>Port</Label>
        </div>
      ...
      <div className={"flex justify-end space-x-4 mt-4 w-full md:mt-0 md:w-48"}>
        {allocation.isDefault ? (
          <Button size={Button.Sizes.Small} disabled>
            Primary
          </Button>
        ) : (
          <>
