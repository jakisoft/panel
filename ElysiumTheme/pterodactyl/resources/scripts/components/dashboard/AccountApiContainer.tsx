    ...
    <PageContentBlock title={"Account API"}>
      ...
      <div css={tw`md:flex flex-nowrap my-10`}>
        <ContentBox
          ...
          css={tw`flex-none w-full md:w-1/2`}
        >
          <CreateApiKeyForm
          ...
          {keys.length === 0 ? (
            ...
            keys.map((key, index) => (
              <GreyRowBox
                key={key.identifier}
                css={[
                  tw`bg-elysium-color3 flex items-center`,
                  index > 0 && tw`mt-2`,
                ]}
              >
                ...
                <p css={tw`text-sm ml-4 hidden md:block`}>
                  <code
                    css={tw`font-mono py-1 px-2 bg-elysium-color4 rounded-2xl`}
                  >
                    {key.identifier}
                  </code>
                </p>
                <Button.Danger
                  css={tw`ml-2 w-8 h-8 transition-transform duration-100`}
                  type={"button"}
