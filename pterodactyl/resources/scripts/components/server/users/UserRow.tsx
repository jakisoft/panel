...
export default ({ subuser }: Props) => {
  ...
  return (
    <GreyRowBox css={tw`mb-2`}>
      ...
      />
      <div
        css={tw`w-10 h-10 rounded-full bg-white border-2 border-neutral-800 overflow-hidden hidden md:block`}
      >
      ...
      <div css={tw`mx-4 hidden md:block`}>
        <p css={tw`font-medium text-center`}>
          {
      ...
      {subuser.uuid !== uuid && (
        <>
          <Can action={"user.update"}>
            <Button.Text
              type={"button"}
              css={tw`block w-8 h-8 transition-transform duration-100 mr-2`}
