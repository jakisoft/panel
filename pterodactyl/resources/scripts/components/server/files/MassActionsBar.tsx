...
const MassActionsBar = () => {
  ...
  const selectedFiles = ServerContext.useStoreState(
  ...
  const onClickConfirmDeletion = () => {
    ...
    deleteFiles(uuid, directory, selectedFiles)
      .then(() => {
        mutate(
          (files) => files.filter((f) => selectedFiles.indexOf(f.name) < 0),
          false
        );
        setSelectedFiles([]);
      })
  ...
  return (
    <>
      ...
      >
        ...
        <Portal>
          <div
            css={tw`pointer-events-none fixed bottom-0 mb-6 flex justify-center w-[120%] z-50`}
          >
            <Fade timeout={75} in={selectedFiles.length > 0} unmountOnExit>
              <div
                css={tw`flex items-center space-x-4 pointer-events-auto rounded-2xl p-4 bg-elysium-color3`}
              >
