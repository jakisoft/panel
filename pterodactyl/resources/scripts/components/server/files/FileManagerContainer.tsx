...
export default () => {
  ...
  const onSelectAllClick = (e: React.ChangeEvent<HTMLInputElement>) => {
    setSelectedFiles(
      e.currentTarget.checked ? files?.map((file) => file.name) || [] : []
    );
  };
  ...
  return (
    <ServerContentBlock title={"File Manager"} showFlashKey={"files"}>
      <ErrorBoundary>
        <div
          className={
            "flex flex-wrap-reverse md:flex-nowrap justify-center mb-4"
          }
        >
        ...
        <>
          {!files.length ? (
            ...
            <CSSTransition classNames={"fade"} timeout={150} appear in>
              <div>
                {files.length > 250 && (
                  <div css={tw`rounded-2xl bg-yellow-400 mb-px p-3`}>
                    <p css={tw`text-neutral-100 text-sm text-center`}>
