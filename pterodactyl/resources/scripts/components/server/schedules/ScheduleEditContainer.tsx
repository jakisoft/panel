...
const CronBox = ({ title, value }: { title: string; value: string }) => (
  <div css={tw`bg-elysium-color2 rounded-2xl shadow-lg p-3`}>
...
const ActivePill = ({ active }: { active: boolean }) => (
  <span
    css={[
      ...
      active ? tw`bg-elysium-green` : tw`bg-elysium-red`,
    ]}
  >
    ...
    <PageContentBlock title={"Schedules"}>
      ...
      {!schedule || isLoading ? (
        ...
        <>
          <ScheduleCronRow
            cron={schedule.cron}
            css={tw`sm:hidden bg-elysium-color2 rounded-2xl shadow-lg mb-4 p-3`}
          />
          <div css={tw`rounded shadow`}>
            <div
              css={tw`sm:flex items-center bg-elysium-color2 p-3 sm:p-6 rounded-2xl shadow-lg`}
            >
              <div css={tw`flex-1`}>
