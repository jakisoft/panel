...
const ButtonStyle = styled.button<Omit<Props, "isLoading">>`
  ${tw`relative inline-block rounded-2xl p-2 uppercase tracking-wide text-sm transition-all duration-150 border`};
  ...
  ${(props) =>
    ...
    css<Props>`
      ${(props) =>
        !props.isSecondary &&
        tw`bg-primary-500 border-primary-600 border text-primary-50`};
  ...
  ${(props) =>
    ...
    css<Props>`
      ...
      &:hover:not(:disabled) {
        ...
        ${(props) =>
          props.color === "red" && tw`bg-red-500 border-red-600 text-red-50`};
        ${(props) =>
          props.color === "primary" &&
          tw`bg-primary-500 border-primary-600 text-primary-50`};
        ${(props) =>
          props.color === "green" &&
          tw`bg-green-500 border-green-600 text-green-50`};
      }
    `};
...
const Button: React.FC<ComponentProps> = ({
  ...
  <ButtonStyle {...props}>
    {isLoading && (
      <div
        css={tw`flex absolute justify-center items-center w-full h-full left-0 top-0`}
      >
        <Spinner size={"small"} />
      </div>
    )}
