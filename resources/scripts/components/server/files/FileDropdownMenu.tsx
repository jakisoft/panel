...
const StyledRow = styled.div<{ $danger?: boolean }>`
  ${tw`p-2 flex items-center rounded-2xl`};
  ${(props) =>
    props.$danger
      ? tw`hover:bg-elysium-red hover:text-red-100`
      : tw`hover:bg-elysium-color3 hover:text-neutral-100`};
`;
...
const FileDropdownMenu = ({ file }: { file: FileObject }) => {
  ...
  const doCopy = () => {
    ...
    copyFile(uuid, joinPath(directory, file.name))
      .then(() => mutate())
  ...
  const doDownload = () => {
    ...
    getFileDownloadUrl(uuid, joinPath(directory, file.name))
      .then((url) => {
