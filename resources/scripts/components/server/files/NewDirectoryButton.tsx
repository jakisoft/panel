...
const NewDirectoryDialog = asDialog({
  ...
  const submit = (
    ...
    createDirectory(uuid, directory, directoryName)
      .then(() =>
        mutate((data) => [...data, generateDirectoryData(directoryName)], false)
      )
      .then(() => close())
      .catch((error) => {
        setSubmitting(false);
    ...
    >
      {({ submitForm, values }) => (
        <>
            ...
            <p css={tw`mt-2 text-sm md:text-base break-all`}>
              ...
              <Code>
                ...
                <span css={tw`text-cyan-200`}>
                  {joinPath(directory, values.directoryName).replace(
                    /^(\.\.\/|\/)+/,
                    ""
                  )}
                </span>
          ...
          <Dialog.Footer>
            <Button.Danger className={"w-full sm:w-auto"} onClick={close}>
              Cancel
            </Button.Danger>
