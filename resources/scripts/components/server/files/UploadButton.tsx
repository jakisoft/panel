...
export default ({ className }: WithClassname) => {
  ...
  const onFileSubmission = (files: FileList) => {
    ...
    if (list.some((file) => !file.size || (!file.type && file.size === 4096))) {
      return addError(
        "Folder uploads are not supported at this time.",
        "Error"
      );
    }
        ...
        >
          ...
          >
            ...
            >
              <div
                className={
                  "flex items-center space-x-4 bg-elysium-color2 w-full ring-4 ring-blue-200 ring-opacity-60 rounded-2xl p-6 mx-10 max-w-sm"
                }
              >
