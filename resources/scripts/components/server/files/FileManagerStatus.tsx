...
const FileUploadList = () => {
  ...
  const cancelFileUpload = ServerContext.useStoreActions(
    (actions) => actions.files.cancelFileUpload
  );
  const clearFileUploads = ServerContext.useStoreActions(
    (actions) => actions.files.clearFileUploads
  );
    ...
    <div className={"space-y-2 mt-6"}>
      {uploads.map(([name, file]) => (
        <div
          key={name}
          className={
            "flex items-center space-x-3 bg-elysium-color3 p-3 rounded-2xl"
          }
        >
          <Tooltip
          ...
          <button
            ...
            className={
              "text-gray-500 hover:text-gray-200 transition-colors duration-75"
            }
          >
...
export default () => {
  ...
  const progress = ServerContext.useStoreState((state) => ({
    uploaded: Object.values(state.files.uploads).reduce(
      (count, file) => count + file.loaded,
      0
    ),
    total: Object.values(state.files.uploads).reduce(
      (count, file) => count + file.total,
      0
    ),
  }));
