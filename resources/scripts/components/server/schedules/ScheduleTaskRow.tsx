...
export default ({ schedule, task }: Props) => {
  ...
  return (
    <div
      css={tw`sm:flex items-center p-3 sm:p-6 my-4 bg-elysium-color2 shadow-lg rounded-2xl`}
    >
      ...
      <div css={tw`flex-none sm:flex-1 w-full sm:w-auto overflow-x-auto`}>
        ...
        {task.payload && (
          <div css={tw`md:ml-6 mt-2`}>
            {task.action === "backup" && (
              <p css={tw`text-xs uppercase text-neutral-300 mb-1`}>
                Ignoring files & folders:
              </p>
            )}
            <div
              css={tw`font-mono bg-elysium-color3 rounded-2xl py-1 px-2 text-sm w-auto inline-block whitespace-pre-wrap break-all`}
            >
              {task.payload}
            </div>
          </div>
      ...
      <div css={tw`mt-3 sm:mt-0 flex items-center w-full sm:w-auto`}>
        {task.continueOnFailure && (
          <div css={tw`mr-6`}>
            <div
              css={tw`flex items-center px-2 py-1 bg-elysium-yellow text-white text-sm rounded-full`}
            >
        ...
        <Can action={"schedule.update"}>
          <Button.Text
            type={"button"}
            css={tw`block w-8 h-8 transition-transform duration-100 text-neutral-500 mr-4 ml-auto sm:ml-0`}
        ...
        <Can action={"schedule.update"}>
          <Button.Danger
            type={"button"}
            css={tw`block w-8 h-8 text-neutral-500 transition-colors duration-150`}
