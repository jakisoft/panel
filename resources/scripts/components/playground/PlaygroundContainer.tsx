...
const customStyles = `
  ...
  .card-glass { background: rgba(255, 255, 255, 0.85); backdrop-filter: blur(10px); border: 1px solid rgba(255, 255, 255, 1); }
...
type VisualCard = { title: string; value: string; description?: string };
...
const Navbar = () => {
    ...
    return (
        <nav className={'fixed top-5 left-1/2 -translate-x-1/2 z-50 w-full max-w-4xl px-4 transition-all duration-500'}>
            <div className={`glass-nav rounded-2xl px-5 flex justify-between items-center shadow-[0_8px_32px_rgba(0,0,0,0.04)] transition-all duration-300 ${
                isScrolled ? 'py-2 scale-95' : 'py-2.5 scale-100'
            }`}>
                <div className={'flex items-center gap-2'}>
                    <div className={'w-7 h-7 bg-gradient-bright rounded-lg flex items-center justify-center text-white'}><Server size={16} /></div>
                    <span className={'font-bold text-lg tracking-tight'}>Elysium <span className={'text-indigo-600'}>Panel</span></span>
                </div>
...
export default () => {
    ...
    const topLeftCard = visualCards[0] ?? { title: 'Total Users', value: '0', description: 'Belum ada data.' };
    const bottomRightCard = visualCards[1] ?? { title: 'Total Servers', value: '0', description: 'Belum ada data.' };
        ...
        <>
                    ...
                    <div className={'lg:col-span-6 relative flex justify-center items-center h-[500px]'}>
                        <div className={'relative w-[300px] h-[300px] md:w-[400px] md:h-[400px] floating'}>
                            ...
                            <div className={'absolute -top-8 -left-16 card-glass p-5 rounded-3xl shadow-2xl max-w-[230px] floating-delayed'}>
                                <p className={'text-[10px] uppercase tracking-widest text-slate-400 font-bold'}>{topLeftCard.title}</p>
                                <h3 className={'text-xl font-extrabold mt-1 mb-2 text-indigo-600'}>{topLeftCard.value}</h3>
                                <p className={'text-[11px] text-slate-500'}>{topLeftCard.description}</p>
                            </div>
                            ...
                            <div className={'absolute -bottom-8 -right-16 bg-white/90 backdrop-blur-xl p-5 rounded-3xl shadow-2xl border border-white/50 max-w-[220px]'}>
                                <p className={'text-[10px] uppercase tracking-widest text-slate-400 font-bold'}>{bottomRightCard.title}</p>
                                <h3 className={'text-xl font-extrabold mt-1 mb-2 text-indigo-600'}>{bottomRightCard.value}</h3>
                                <p className={'text-[11px] text-slate-500'}>{bottomRightCard.description}</p>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <section id={'pricing'} className={'py-24 px-6'}>
                <div className={'max-w-7xl mx-auto'}>
                    <div className={'text-center mb-16'}>
                        <ScrollReveal>
                            <h2 className={'text-3xl md:text-4xl font-extrabold mb-4 tracking-tight'}>{pricingTitle}</h2>
                            <p className={'text-slate-500 text-[14px]'}>{pricingSubtitle}</p>
                        </ScrollReveal>
                    </div>
                    ...
                    <div className={'grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6'}>
                        {(pricingItems.length ? pricingItems : [{ name: 'Starter', price: 'Rp 15.000/bulan', description: 'Paket dasar panel.', features: ['Support cepat'] }]).map((item, idx) => (
                            <div key={`${item.name}-${idx}`} className={'bg-white rounded-[28px] p-8 border border-slate-100 shadow-[0_20px_40px_rgba(0,0,0,0.04)]'}>
                                ...
                                <p className={'text-indigo-600 font-black text-2xl mt-2'}>{item.price}</p>
                                <p className={'text-slate-400 text-[13px] mt-3 mb-6'}>{item.description}</p>
                                <ul className={'space-y-3'}>
                                    {(item.features ?? []).map((feature) => (
                                        <li key={feature} className={'flex items-center gap-2 text-[13px] text-slate-600'}><Check size={15} className={'text-green-500'} /> {feature}</li>
                                    ))}
                                </ul>
                            </div>
                        ))}
            ...
            <section id={'faq'} className={'py-24 px-6 bg-white'}>
                <div className={'max-w-3xl mx-auto'}>
                    <div className={'text-center mb-16'}>
                        <ScrollReveal>
                            <span className={'text-indigo-600 font-bold text-[11px] uppercase tracking-widest bg-indigo-50 px-3 py-1 rounded-full'}>{faqBadge}</span>
                            <h2 className={'text-3xl md:text-4xl font-extrabold mt-4 tracking-tight'}>{faqTitle}</h2>
                            <p className={'text-slate-500 text-[14px] mt-4'}>{faqSubtitle}</p>
                        </ScrollReveal>
                    </div>

                    <div className={'space-y-4'}>
                        {(faqItems.length ? faqItems : [{ question: 'Apakah panel ini aman?', answer: 'Ya, aman untuk penggunaan harian.' }]).map((item, idx) => (
            ...
            <footer className={'bg-slate-900 pt-20 pb-10 px-6 text-white'}>
                <div className={'max-w-7xl mx-auto'}>
                    <div className={'grid grid-cols-1 md:grid-cols-3 gap-12 mb-16'}>
                        <div>
                            <div className={'flex items-center gap-2'}><Github size={16} /><span className={'font-extrabold text-xl'}>Elysium Theme</span></div>
                            <p className={'text-slate-400 text-[13px] mt-4'}>Custom interface untuk Pterodactyl Panel.</p>
                        </div>
                        <div>
                            <h4 className={'font-bold text-[14px] mb-4 uppercase tracking-widest text-indigo-400'}>Panel</h4>
                            <ul className={'space-y-2 text-slate-400 text-[13px]'}>{['Server Management', 'Monitoring', 'Backup & Restore'].map((l) => <li key={l}>{l}</li>)}</ul>
                        </div>
                        <div>
                            <h4 className={'font-bold text-[14px] mb-4 uppercase tracking-widest text-indigo-400 flex items-center gap-2'}><Sparkles size={14} /> Connect</h4>
                            <div className={'flex gap-3'}>{[Twitter, Mail, Instagram, Globe].map((Icon, idx) => <a key={idx} href={'#'} className={'w-10 h-10 rounded-full bg-white/5 flex items-center justify-center'}><Icon size={16} /></a>)}</div>
                        </div>
                    </div>
                </div>
            </footer>
